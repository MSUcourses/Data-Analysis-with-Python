...
Данный пункт предназначен как краткий справочник для запуска вашего первого кода.
Для подробного описания смотрите следующие пункты. 
...
дополнительных приложений/программ и т.д. Все, что вам требуется — подключение в интернет 
и Google аккаунт.
...
Как и с любым другим документом на Google Drive, вы можете комментировать,
делиться и редактировать ваш блокнот вместе с другими пользователями. Но, помимо этого,
есть некоторые особенности, которые лучше освоить перед началом работы.
...
1. __Копирование на свой диск__
   ...
   - Нажмите на `Копировать на Диск` или перейдите в `Файл` и нажмите 
     ...
     > На диске создаться файл под названием Colab Notebooks, в котором
...
3. __Аппаратный ускоритель__
   ...
   Для обычных задач смысла в использовании нет:
   - Перейдите `Изменить → Настройки блокнота`;
   - В окне `Аппаратный ускоритель` выберите `GPU` или `TPU` и нажмите `Сохранить`.
      > На самом деле GPU и TPU является одной и той же технологией. Единственное
...
Перед тем, как переходить к следующим пунктам, необходимо установить сам язык,
на котором будем писать весь код, а именно — [Python](https://www.python.org/about/).
...
- После чего напишите:
    ...
    <img width="400" alt="Screenshot 2021-07-03 at 12 09 57" src="https://user-images.githubusercontent.com/28728575/125428576-7f382c8c-1950-4a77-abf5-d9d7193abe0e.png">
...
### IDLE (Integrated DeveLopment Environment) <a name="IDLE"></a>

После установки _Python_ у вас должно появиться **IDLE** — упрощенный редактор, который отлично подходит для начала программирования
...
Для полноценной работы с языком Python вам необходимы библиотеки (далее пакеты), которые могут упросить написание 
...
(Pip Installs Packages — рекурсивный акроним).
...
#### Основные команды
Для полного наборы команд рекомендуем посетить [официальную документацию][8].
- Установка пакетов:
    ```shell
...
Крайне удобный инструмент не просто для программирования, а для создания красивых
визуальных проектов с сопровождением графиков, изображений, комментариев и т. д. 
Поэтому данная среда получила широкое внимание в области науки о данных, 
...
> JupyterLab. Рассматривать его мы не будем, так как он имеет меньший функционал.
...
1. __Блокнот__
    ...
    В режиме _Notebook_ все свойства аналогичны с Colab.  
