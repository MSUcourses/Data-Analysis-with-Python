...
_Яндекс.Контест_ — сервис для онлайн-проверки заданий по программированию. Он предназначен для проведения состязаний любого уровня: олимпиады, соревнования международного класса, домашиние задания. Его можно использовать также для подготовки к турнирам и для приёма экзаменов. Решения проверяются автоматически — с помощью набора тестов, составленных авторами заданий. Участники отправляют свои решения в тестирующую систему, а та выдает результат.

Мы предлагаем ознакомиться с краткой инструкцией по данной платформе, чтобы вы могли уделить свое время только на решения задач.


## Инструкция

В конце каждой прослушенной лекции будут задачи на дом на данной платформе. Давайте на примере [нулевой лекции](https://contest.yandex.ru/contest/29359/standings) разберемся, что к чему.

Перейдя по [ссылке](https://contest.yandex.ru/contest/29359/standings), вам в первую очередь нужно будет авторизоваться на платформе через аккаунт в Яндексе.
...
После старта вы увидите основую инфомарцию о соревновании: название, количество оставшегося времени, кнопка досрочного завершения. В основном будет интересовать ссылки для решения задач.
...
Есть два варианта _вардикта_ после отправки вашего ответа: 
...
Также есть другие вердикты, которые могут встретиться (очень редко) у вас при решении задач. Подробнее про них вы можете прочитать [здесь](https://contest.yandex.ru/errors/).
